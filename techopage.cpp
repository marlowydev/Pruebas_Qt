...
TechoPage::TechoPage(QWidget *parent)
    : QWidget(parent)
{
    QLabel *title = new QLabel(tr("Cuantificación de Agua y Espumas - Techo Flotante"), this);
    title->setObjectName("pageTitle");
    title->setAlignment(Qt::AlignCenter);

    QPushButton *backButton = new QPushButton(tr("< Volver al menu"), this);
    backButton->setObjectName("backButton");
    connect(backButton, &QPushButton::clicked, this, &TechoPage::backRequested);
    ...
    connect(m_diametro, &QLineEdit::textChanged, this, &TechoPage::recalcular);
    connect(m_metrosRadio, &QRadioButton::toggled, this, &TechoPage::recalcular);
    connect(m_piesRadio, &QRadioButton::toggled, this, &TechoPage::recalcular);
    connect(m_ia, &QLineEdit::textChanged, this, &TechoPage::recalcular);
    connect(m_tiempo, &QLineEdit::textChanged, this, &TechoPage::recalcular);
    connect(m_pe, &QLineEdit::textChanged, this, &TechoPage::recalcular);
    connect(m_pa, &QLineEdit::textChanged, this, &TechoPage::recalcular);

    recalcular();
}

QLineEdit *TechoPage::createLineEdit(double min, double max, int decimals)
{
...
void TechoPage::showEvent(QShowEvent *event)
{
    QWidget::showEvent(event);
    m_diametro->setFocus();
}

QLabel *TechoPage::createResultLabel()
{
...
void TechoPage::recalcular()
{
    ...
    const double area = (dPies - RESTA_TECHO) * 2.0 * PI;
    ...
    if ((pe > 0.0 || pa > 0.0) && qAbs(sumaPorcentajes - 100.0) > 0.01) {
        m_warning->setText(tr("Atención: PE + PA = ") + QString::number(sumaPorcentajes, 'f', 1)
                           + tr("%. La suma debe ser 100% para que GE + GA = GA/E."));
    } else {
        m_warning->clear();
    }
}
