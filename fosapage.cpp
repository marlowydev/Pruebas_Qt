...
FosaPage::FosaPage(QWidget *parent)
    : QWidget(parent)
{
    QLabel *title = new QLabel(tr("Cuantificación de Agua y Espumas - Fosas"), this);
    title->setObjectName("pageTitle");
    title->setAlignment(Qt::AlignCenter);

    QPushButton *backButton = new QPushButton(tr("< Volver al menu"), this);
    backButton->setObjectName("backButton");
    connect(backButton, &QPushButton::clicked, this, &FosaPage::backRequested);
    ...
    iaLabel->setToolTip(tr("Valor fijo IA = 0.16"));
    ...
    formLayout->addRow(tr("Índice IA:"), iaLabel);
    formLayout->addRow(tr("Tiempo (min):"), m_tiempo);
    formLayout->addRow(tr("Espuma PE (%):"), m_pe);
    formLayout->addRow(tr("Agua PA (%):"), m_pa);

    QLabel *hint = new QLabel(tr("Complete cada campo: largo y ancho en metros, tiempo en minutos, y PE / PA en porcentaje (ej. 3 = 3%)."), dataGroup);
    ...
    connect(m_largo, &QLineEdit::textChanged, this, &FosaPage::recalcular);
    connect(m_ancho, &QLineEdit::textChanged, this, &FosaPage::recalcular);
    connect(m_tiempo, &QLineEdit::textChanged, this, &FosaPage::recalcular);
    connect(m_pe, &QLineEdit::textChanged, this, &FosaPage::recalcular);
    connect(m_pa, &QLineEdit::textChanged, this, &FosaPage::recalcular);

    recalcular();
}

QLineEdit *FosaPage::createLineEdit(double min, double max, int decimals)
{
...
void FosaPage::showEvent(QShowEvent *event)
{
    QWidget::showEvent(event);
    m_largo->setFocus();
}

QLabel *FosaPage::createResultLabel()
{
